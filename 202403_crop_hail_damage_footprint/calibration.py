# -*- coding: utf-8 -*-
"""
Empirical calibration functions as basis for
- spatially explicit vulnerability functions
- the computation of skill scores for damage footprint detection

The key functionality here is to link damages, exposures and hazard per grid cell.
A lot of the functionality here is not needed for the crop hail damage footprint
To ensure complete functionality we left also the parts not directly relevant for the study at hand.
...
#value ranges and cut offs for MESHS and POHs
INT_RANGE_DICT = {
...
#dates to drop because harvest has already occurred
...
def assign_centroids_gdf(gdf, hazard, distance='euclidean',
    ...
    -1 used for distances > threshold in point distances. If raster hazard,
