...
hab_years = ['1992', '2020']
basins = ['AP', 'WP', 'IO', 'SH']
output = '/cluster/work/climate/shuelsen/manuscript/results/intermediate/'

# create exposure files for both years per basin
for basin in basins:
    for pop_year in pop_years:
        # Load data
        pop_path = f'/cluster/work/climate/shuelsen/manuscript/data/worldpop/wp_{pop_year}_{basin}.shp'
        ...
        # save exposure data
        exp_pop.write_hdf5(f'{output}exposure/exp_wp_{basin}_{pop_year}.hdf5')
...
for basin in basins:
    for pop_year in pop_years:
        for hab_year in hab_years:
            # Load data
            rhab_path = f'/cluster/work/climate/shuelsen/manuscript/data/rhab/basin_clips/rhab_{hab_year}_{basin}.shp'
            pop_path = f'/cluster/work/climate/shuelsen/manuscript/data/worldpop/wp_{pop_year}_{basin}.shp'
            pop = gpd.read_file(pop_path)
            rhab = gpd.read_file(rhab_path)
            ...
            join_filter = join_filter.drop_duplicates(subset='exp_index', keep='first')

            # save data
            join_filter.to_csv(f'{output}rhab_exposure/per_basin/exp_rhab_{hab_year}_wp_{pop_year}_{basin}.csv')

# create global merged habitat and population files
for pop_year in pop_years:
    for hab_year in hab_years:
        # load files
        AP = pd.read_csv(f'{output}rhab_exposure/per_basin/exp_rhab_{hab_year}_wp_{pop_year}_AP.csv')
        WP = pd.read_csv(f'{output}rhab_exposure/per_basin/exp_rhab_{hab_year}_wp_{pop_year}_WP.csv')
        IO = pd.read_csv(f'{output}rhab_exposure/per_basin/exp_rhab_{hab_year}_wp_{pop_year}_IO.csv')
        SH = pd.read_csv(f'{output}rhab_exposure/per_basin/exp_rhab_{hab_year}_wp_{pop_year}_SH.csv')

        # concatenate basins
        basins_all = pd.concat([AP, WP, IO, SH])
        basins_all.to_csv(f'{output}rhab_exposure/wp{pop_year}_rhab{hab_year}.csv')
