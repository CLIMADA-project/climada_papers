...
save_bs_vars = ['PAA','MDD','MDR','count_cell']
comp = dict(zlib=True, complevel=5)
encoding = {var: comp for var in save_bs_vars}
ds_boot_path = f"{data_dir}/out_files/calib_emp/bootstrap/{c_path}_boot.nc"
ds_boot_roll[save_bs_vars].to_netcdf(ds_boot_path,encoding=encoding)

# Cross validation
out_tuple_CV = sc.bootstrapping(ds, ds_roll,haz_var,5,intensity_range,
                           log_fit=False,cut_off=cut_off,keep_raw_values=True,
                           do_cross_validation=True,by_year=True)
ds_CV,ds_CV_roll,ds_CV_roll_cut, fit_data, CV_dates,CV_years = out_tuple_CV
...
if haz_var=='MESHS' or haz_var == 'MESHS_4km' or haz_var=='MESHS_opt_corr':
...
df_boot_emanuel,df_boot_params = sc.calib_opt.fit_emanuel_to_bootstrap(
    ds_boot,impf_emanuel,pbounds,opt_var='MDR',plot='quant')

# Cross validation
df_CV, df_CV_params = sc.calib_opt.fit_emanuel_to_bootstrap(
    ds_CV,impf_emanuel,pbounds,b_sample_name='CV_split',opt_var='MDR',plot='all')
...
#Save sigmoidal bootstrap functions
df_boot_params.to_csv(f'{data_dir}/out_files/calib_emp/bootstrap/{c_path}_boot_params.csv')
df_boot_paramsPAA.to_csv(f'{data_dir}/out_files/calib_emp/bootstrap/{c_path}_boot_paramsPAA.csv')
df_boot_emanuel.to_csv(f'{data_dir}/out_files/calib_emp/bootstrap/{c_path}_boot_emanuel.csv')
df_boot_emanuel_PAA.to_csv(f'{data_dir}/out_files/calib_emp/bootstrap/{c_path}_boot_emanuelPAA.csv')


# Cross validation
df_CV_PAA, df_CV_paramsPAA = sc.calib_opt.fit_emanuel_to_bootstrap(
    ds_CV,impf_emanuelPAA,pboundsPAA,b_sample_name='CV_split',opt_var='PAA',plot='all')

#Save cross-validation results
CV_years.to_csv(f'{data_dir}/out_files/calib_emp/CV/{c_path}_CV_years.csv')
df_CV_params.to_csv(f'{data_dir}/out_files/calib_emp/CV/{c_path}_CV_params.csv')
df_CV_paramsPAA.to_csv(f'{data_dir}/out_files/calib_emp/CV/{c_path}_CV_paramsPAA.csv')
