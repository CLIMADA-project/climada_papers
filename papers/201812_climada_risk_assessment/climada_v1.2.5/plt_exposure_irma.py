...
from climada.entity import BlackMarble, IFTropCyclone, ImpactFuncSet, add_sea
...
def exposures():
    ...
    for cntry in CNTRIES:
        ...
        if cntry ==  'Turks And Caicos Islands':
            nord_isl.append(ent.rename(columns={"if_": "if_TC"}))
        else:
            
            south_isl.append(ent.rename(columns={"if_": "if_TC"}))
    
    nord_isl = pd.concat(nord_isl)
    nord_isl = add_sea(nord_isl, (10.0, 0.5))
    south_isl = pd.concat(south_isl)
    south_isl = add_sea(south_isl, (10.0, 0.5))
...
def irma_best_track():
    data_irma = TCTracks()
    data_irma.read_ibtracs_netcdf(provider='usa', storm_id='2017242N16333')
    return data_irma

def plot_left(exp, data_irma, tc_irma, ax, scale_pos, cntour_loc, label_loc):
    """ Plot exposed value, irma track and irma wind field. """
    extent = [exp.longitude.min() - BUFFER_DEG, exp.longitude.max() + BUFFER_DEG, 
              exp.latitude.min() - BUFFER_DEG, exp.latitude.max() + BUFFER_DEG]
    ax.set_extent((extent))
    u_plot.add_shapes(ax)

    sel_pos = np.argwhere(exp.value>0)[:,0]
    ax.hexbin(exp.longitude[sel_pos], exp.latitude[sel_pos], C=exp.value[sel_pos], 
    ...
    ax.clabel(cs, inline=1, fontsize=10, manual=cntour_loc, fmt='%1.0f')
    tc_irma.intensity /= MS2KN
...
def plot_percen(irma_tc, exp, if_exp, axs):
    ...
    im = axs.hexbin(exp.longitude[sel_exp], exp.latitude[sel_exp], 
...
def fig03_fig04(data_dir, fig_dir=None):
    """ Generate figures 03 and 04 """
    south_isl, nord_isl = exposures()
    
    data_irma = irma_best_track()
