...
if try_read_from_tiff and os.path.exists(os.path.join(RES_DIR, '%s_000_%ias.tiff' %(files[0][0:-8], RES_ARCSEC))):
    ...
    for idx, fi in enumerate(files):
        ...
        if try_read_from_tiff and os.path.exists(os.path.join(RES_DIR, '%s_%ias.tiff' %(fi[0:-4], RES_ARCSEC))):
            print('\n' + '\x1b[1;03;30;30m' + 'Loading: %s_%ias.tiff' %(fi[0:-4], RES_ARCSEC) + '\x1b[0m')
    ...
    if write_to_tiff:
        ...
        exposure_data.plot_raster(res=RES_ARCSEC/3600, save_tiff=\
            os.path.join(RES_DIR, '%s_000_%ias.tiff' %(files[0][0:-8], RES_ARCSEC)))
        exposure_data.plot_raster(res=RES_ARCSEC/3600, raster_res=res_target/3600, save_tiff=\
            os.path.join(RES_DIR, '%s_000_%ias.tiff' %(files[0][0:-8], res_target)))     

# exposure_data = Exposures()
# try_read_from_tiff = True        
# if try_read_from_tiff and os.path.exists(os.path.join(RES_DIR, '%s_000_%ias.tiff' %(files[0][0:-8], res_target))):
#     print('Loading: %s_000_%ias.tiff' %(files[0][0:-8], res_target))
#     exposure_data.set_from_raster(os.path.join(RES_DIR, '%s_000_%ias.tiff' %(files[0][0:-8], res_target)))
# else:
#     grid_stats = pd.DataFrame(index=np.arange(0, len(files)), columns=['country', 'grid_count', 'sum', 'max', 'mean', 'median'])
#     for idx, fi in enumerate(files):
#         exposure_tmp = Exposures()
#         if try_read_from_tiff and os.path.exists(os.path.join(RES_DIR, '%s_%ias.tiff' %(fi[0:-4], res_target))):
#             print('Loading: %s_%ias.tiff' %(fi[0:-4], res_target))
#             exposure_tmp.set_from_raster(os.path.join(RES_DIR, '%s_%ias.tiff' %(fi[0:-4], res_target)))
#         exposure_data = exposure_data.append(exposure_tmp)
# exposure_data = Exposures(exposure_tmp)
# exposure_data.set_geometry_points()
# exposure_data.check()
# if write_to_tiff:
#     exposure_data.plot_raster(res=res_target/3600, save_tiff=\
#         os.path.join(RES_DIR, '%s_000_%ias.tiff' %(files[0][0:-8], res_target)))           
